...
全称是 AbstractQueuedSynchronizer，是阻塞式锁和相关的同步器工具的框架，其他的一些并发工具也都是依赖这个AQS的，如果理解了AQS的原理，有助于理解其他并发工具原理的理解。下面，我们就来看下什么是AQS。

AQS全称翻译过来就是抽象的队列同步器，其他的同步器都是它的子类，都调用了这个父类里面相关的方法，并且它实现的锁是这种阻塞式的锁，更加类似于synchronized。
...
- 用 state 属性来表示资源的状态，资源的状态分为独占模式和共享模式，独占模式只允许一个线程能够访问共享资源，共享模式允许多个线程能够访问资源，但是可能会提供一个访问的上限，比如允许十个线程访问共享资源，多的就不允许访问，这种就叫做共享模式；子类需要定义如何维护这个状态，控制如何获取 锁和释放锁，比如刚开始state等于0表示没有人占用这个锁，将来将state表示1就表示有线程来占用这个锁，这里就是用state属性来表示你是获取了锁还是释放了锁，其实就是把state在0和1之间来回变化
  - getState - 获取 state 状态 
  - setState - 设置 state 状态 
  - compareAndSetState - cas 机制设置 state 状态，这里并不是利用state去加锁，是为了防止多个线程都来修改state状态的时候，你这个state的线程安全，使用cas重试的方式来保证state变量的原子性。
  - 独占模式是只有一个线程能够访问资源，而共享模式可以允许多个线程访问资源 
- 提供了基于 FIFO 先进先出的等待队列，比如有一个线程已经占用了这个独占锁了，那么其他的线程就只能等待，这个队列就是由AQS来维护的，当然这个等待队列类似于 Monitor 的 EntryList，只不过Monitor的EntryList是在c++的层面上来实现的，而我们的AQS是纯java来实现的。
- 条件变量来实现等待wait、唤醒notify机制，支持多个条件变量，里面的某一个条件变量类似于 Monitor 的 WaitSet。
...
// 如果获取锁失败
if (!tryAcquire(arg)) {
    // 进入等待队列进行等待, 阻塞当前线程，AQS里面让当前线程暂停还是恢复运行用的是park()和unpark()方法
}
...
class MyLock implements Lock {
    /*同步器锁MySync，将来MyLock锁的大部分功能，都是下面的代码来完成的*/
    ...
    @Override  /* 加锁，一次不成功就会进入等待队列进行等待 */
    public void lock() {
        sync.acquire(1);
    }

    @Override  /* 可以在加锁的过程中，其他线程可以来打断 */
    public void lockInterruptibly() throws InterruptedException {
        sync.acquireInterruptibly(1);
    }

    @Override  /* 尝试加锁，一次加锁不成功，就返回false，就不会继续死等 */
...
###### **ReentrantLock下面的非公平锁**

```java
    public ReentrantLock() {
        //ReentrantLock下面有非公平锁
...
```
acquire()方法里面首先调用了tryAcquire()方法，tryAcquire()方法其实就是尝试去加锁， 就是刚刚thread-1又去重试一次；接下来就会进入addWaiter(Node.EXCLUSIVE)方法
```
...
![1681625204054](D:\C3D\juc\dark-horse-juc-notes\146集之后的内容\146集之后的内容.assets\1681625204054.png)

![1681625404406](D:\C3D\juc\dark-horse-juc-notes\146集之后的内容\146集之后的内容.assets\1681625404406.png)



```java
    // ㈣ AQS 继承过来的方法, 方便阅读, 放在此处
    // addWaiter()方法就是构造一个node队列，可以看下面的图，一个head和tail都是没有东西的，现在有竞争了，就会去尝试添加一个节点，在head后面添加一个节点，是一个双向链表，head指向一个节点，这个节点又指向下一个节点；需要注意的是：首次去创建node的时候会创建两个，第一个node为dummy(哑元)或则哨兵，是用来占位的，并不关联任何的线程；只有第二个node才是thread-1,因为thread-1竞争失败了，就关联到一个node上面，因为thread-1是第一个加入的，所以关联到链表的尾部。这就是一个线程关联到node对象上，node对象有属于非公平锁的链表上，如上图所示这其实就是一个等待队列，需要注意的是这个线程还是活动的，相当于是addWaiter已经把这个node对象给创建好了，接着就会进入acquireQueued()这个方法
...
![1681626702276](D:\C3D\juc\dark-horse-juc-notes\146集之后的内容\146集之后的内容.assets\1681626702276.png)
...
![1681627562594](D:\C3D\juc\dark-horse-juc-notes\146集之后的内容\146集之后的内容.assets\1681627562594.png)

```java
    // ㈧ 阻塞当前线程
    // 就是把当前线程park，阻塞住，执行到这里这个node(thread-1)就变成了灰色的，灰色的表示thread-1已经阻塞住了，这是关于竞争失败一个状态的转换，
...
![1681627595837](D:\C3D\juc\dark-horse-juc-notes\146集之后的内容\146集之后的内容.assets\1681627595837.png)

假设现在有多个线程竞争失败了，就变成了上图所示的样子，注意看他们构成了一个双向的链表，头部是我们没有关联线程的节点，下一个是thread-1，然后是thread-2、thread-3；可以看到除去最后一个，其waitstatus的状态都被改为了-1，意思是它由上一个来唤醒，也就是-1有责任去唤醒它的后继节点，就变成这个样子了，下面我们再来看下释放锁的流程。

----

释放锁的流程就是thread-0终于执行完了，会把exclusiveOwnerThread设置为null，会把立面的state设置为0；那我们来看下释放锁的流程，看下release()函数。
...
![1681629441449](D:\C3D\juc\dark-horse-juc-notes\146集之后的内容\146集之后的内容.assets\1681629441449.png)

release()方法里面首先要tryRelease( )方法，tryRelease()方法比较简单，就是把setExclusiveOwnerThread()方法社值为null，然后把setState()方法设置为0，这一块的逻辑在可重入哪里进行讲解。

tryRelease()方法通过之后，会去检查head是不是为null，然后再看下head.waitStatus是不是不等于0，确实不等于0，是-1呀，这个时候就会去唤醒下一个节点，就进入了这个unparkSuccessor(h)这个流程，unparkSuccessor(h) 是后继的意思呀，是去唤醒后继的节点。我们来看unparkSuccessor()是怎么唤醒的
...
![1681629656520](D:\C3D\juc\dark-horse-juc-notes\146集之后的内容\146集之后的内容.assets\1681629656520.png)

unparkSuccessor()方法用于唤醒后继节点，就会找到离head最近的一个node，也就是找到thread-1这个node，然后unpark让它恢复运行，底层就是调用的LockSupport.unpark(s.thread)，就是将线程恢复运行，大家想恢复运行了，这个线程就有机会去竞争锁，如果没有其他线程和它竞争，那么thread-1就成为了exclusiveOwnerThread，并且也把state改为了1。

刚刚线程1 thread-1是在parkAndCheckInterrupt()方法这里阻塞着的，那么一运行就又进去循环了，去执行if(p==head && tryAcquire(arg))代码，成功了之后，thread-1就会变成owner线程，并且把state变为1。
...
![1681630573363](D:\C3D\juc\dark-horse-juc-notes\146集之后的内容\146集之后的内容.assets\1681630573363.png)

刚刚只是分析了一种情况，也就是你的被唤醒的thread-1 就它一个人，没有人跟它竞争，那么它自然可以成功获得到锁，但是我们这里是非公平锁，非公平锁的意思就是如果这个时候又来了一个线程4，线程4并不是在等待队列里面的，是一个新的线程，这个线程也想要获得锁，就会跟thread-1线程发生一个竞争，要竞争去成为这个owner，如果不巧这个thread-4去把state改为1了，那么这个thread-4就变成owner了，这个时候相当于你的thread-1又失败了，失败了之后就会再次进入这个if(p==head && tryAcquire(arg)) 流程，那么tryAcquire(arg)就是一个false，false以后就会进去下面的shouldParkAfterFailedAcquire() 流程，又阻塞住，虽然被唤醒，但是竞争失败的一种情况。
...
把state从0改成了1，然后把owner线程改为了自己，这就是加锁成功的情况。

![image-20220314153311208](assets/image-20220314153311208-1678605691096.png)

第一个竞争出现时

像刚才那样，ownerThread里面是Thread-0,并且state是1，这个时候又来了一个thread-1，thread-1已经不能将state从0改为1了，因为state已经是1了，所以就会进入else代码块，如下图所示：
...
![image-20220314153333551](assets/image-20220314153333551-1678605691096.png)
...
![image-20220314153434087](assets/image-20220314153434087-1678605691096.png)

当前线程进入 acquireQueued 逻辑 

1. acquireQueued 会在一个死循环中不断尝试获得锁，失败后进入 park 阻塞
2. 如果自己是紧邻着 head（排第二位），那么再次 tryAcquire 尝试获取锁，当然这时 state 仍为 1，失败
3. 进入 shouldParkAfterFailedAcquire 逻辑，将前驱 node，即 head 的 waitStatus 改为 -1，这次返回 false![image-20220314153526384](assets/image-20220314153526384-1678605691096.png)
4. shouldParkAfterFailedAcquire 执行完毕回到 acquireQueued ，再次 tryAcquire 尝试获取锁，当然这时 state 仍为 1，失败
5. 当再次进入 shouldParkAfterFailedAcquire 时，这时因为其前驱 node 的 waitStatus 已经是 -1，这次返回 true
6. 进入 parkAndCheckInterrupt， Thread-1 park（灰色表示）

![image-20220314153708216](assets/image-20220314153708216-1678605691096.png)

再次有多个线程经历上述过程竞争失败，变成这个样子

![image-20220314154029099](assets/image-20220314154029099-1678605691096.png)
...
![image-20220314153831407](assets/image-20220314153831407-1678605691096.png)
...
![image-20220314153919958](assets/image-20220314153919958-1678605691096.png)
...
![image-20220314154048958](assets/image-20220314154048958-1678605691096.png)
...
我们都知道reentrantLock是可重入的，那它是怎么保证可重入的嘞？

我们看下面获取锁和释放锁的相关代码，还是以非公平锁为例，非公平锁是调用的nonfairTryAcquire()方法，传过来的acquires 参数就是一个1，首先会去看看状态是不是0，如果是0的话就表示还没有别人获得过锁，没有获得过锁就去compareAndSetState( 0,acquires )方法，将acquires试图从0改为1，如果成功了的话，就把owner线程设置为当前线程，返回true；如果你同一个线程下次又去调用了nonfairTryAcquire()方法，会怎么样嘞?

显然c==0这个条件已经不成立了，因为getState()返回的肯定是1，是1的话就去做一个判断，判断当前线程是不是等于owner线程，如果当前线程就是owner线程，那么这个时候就发生了锁重入，发生锁重入其实就是将state状态++，你原来是1，刚开始把它改为1了表示占用了锁，那我在这个基础上再次加1，表示我重入了两次了，也就是我这个线程曾经调用了两次获取这个锁，那么相当于锁重入发生时，把state做一个累加。

----

那释放的时候会怎么做嘞？释放的时候会去调用tryRelease( releases )方法，里面有c=getState() - releases  ; 也就是去减去releases ，同样这个releases 也是1，每次去减1，比如现在的state现在是2，那么就2-1，得到这个c是1，但是c是1的情况下，我应不应该把state置为0，把owner也设置为null嘞，不应该，这种情况就会返回一个false，表示我只是让这个锁重入的计数-1了，我没有真正把这个锁释放掉；直到这个state设置为1，那么再次去减1，已经变为0了，这个条件成立了，那么就会把free变量设置为true，然后把setExclusiveOwnerThread()设置为null。然后返回true，表示我真正释放成功了，释放成功之后这个true用于判断是不是要接下来去唤醒那些阻塞的线程，因为你只有是在真正把锁释放完了，计数都减为0了，我再去唤醒其他线程才有意义。
