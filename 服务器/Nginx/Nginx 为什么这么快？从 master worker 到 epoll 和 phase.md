...
>
> 本文共计xx字，适合有一定 Nginx 基础的读者，整理不易，点个免费的赞吧❤️





Nginx 为什么快？一句话回答：**多进程 + 异步非阻塞 + 事件驱动 + 模块化**。
...
- **一个 master，多个 worker**——master 不处理请求，只管 worker；worker 才是干活的人
- **每个 worker 一个 epoll 实例**——单线程、异步处理成千上万个连接
- **共享内存**让 worker 之间共享状态（缓存元数据、限流计数、SSL 会话）。worker 自身的内存是隔离的
...
1. 读配置文件
2. fork 出 worker 进程
3. 监听信号（HUP/USR1/USR2/TERM/QUIT），把信号转发给 worker
4. 监控 worker，挂了就重启
...
如果你写过 Tomcat 或者 Apache prefork，会有个疑惑：**为什么 nginx 不开一堆线程并发处理请求？**

理由：

- **多线程要锁**。锁就要竞争，竞争就有 cache miss 和上下文切换开销
- **多线程一旦阻塞会卡住整个进程**——一个文件 IO 卡住，那个线程就堵了
- **nginx 用异步非阻塞**——一个 worker 就能同时处理几万连接，根本不需要线程并发

简单说：nginx 的每个 worker 是个**单线程的事件循环**，靠 epoll + 非阻塞 IO 在并发模型上吊打多线程。后面会展开。
...
2. **`reuseport`**（新办法）：内核在 listen socket 上做负载均衡，每个 worker 只被唤醒处理分给自己的连接
...
select/poll/epoll 都是 Linux 上的 IO 多路复用机制——**让一个进程同时监听多个 fd，谁有事件就处理谁**。三者有性能差异：
...
- 红黑树：按 key 的 MD5 快速查找
- LRU 链表：每访问一次就把节点移到链表头
...
Linux 默认临时端口范围是 32768-60999，**一共两万八**。一旦 nginx → 后端的 TCP 连接太多（即使是 TIME_WAIT 状态），就会**端口耗尽**，新请求直接连不上后端。
