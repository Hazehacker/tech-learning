...
> | const  | 常量查询[只返回一条数据]（多见）                             | **主键或唯一索引 等值查询**，<u>数据唯一且确定</u> | ⭐⭐⭐⭐⭐         |          |
> | eq_ref | **非主键的唯一索引查询，或两个表的等值连接**[只返回一条数据] | 非主键的**唯一索引查询**，或两个表的等值连接       | ⭐⭐⭐⭐☆         |          |
> | ref    | 使用**普通索引进行等值查询**                                 | 普通索引匹配，可能返回多条记录                     | ⭐⭐⭐☆☆         |          |
> | range  | **范围查询**，<u>走索引但范围扫描</u>                        | 如 `BETWEEN`, `IN`, `>`, `<` 等操作                | ⭐⭐☆☆☆         | 最低要求 |
