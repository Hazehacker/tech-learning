...
RabbitMQ 的五种经典工作模式，本质上是基于 **Exchange（交换机）的类型** 以及 **消费者数量** 的不同组合。

### 1. 简单模式（Simple / 点对点）

**工作原理：**
最基础的模型。一个生产者（P）将消息发送到一个队列（Queue），只有==一个消费者（C）监听并消费一个队列==。消息一旦被消费，就从队列中删除。

- **结构**：P -> Queue -> C（1对1）

> 🗣️ **面试回答（几句话讲清楚）：**
> “简单模式是最基础的**一对一**模型。一个生产者发送消息到一个队列，只有一个消费者监听并消费。它主要用于简单的**异步解耦**，比如用户注册后异步发送一条欢迎短信。”



### 2. 工作队列模式（Work Queues / 争抢模式）

**工作原理：**
当任务过重，单个消费者处理不过来时，可以部署多个消费者。==多个消费者（C1, C2...）共同监听同一个队列，**竞争**获取消息==。

- **分发策略**：默认是**轮询分发**（Round Robin，平均分配）；实际生产中通常配置为**公平分发**（Fair Dispatch，通过 `prefetch_count` 限制消费者每次拉取的消息数，能者多劳）。
- **结构**：P -> Queue -> C1, C2...（1对多竞争）

> 🗣️ **面试回答（几句话讲清楚）：**
> “工作队列模式是**一对多**的竞争消费模型。多个消费者监听同一个队列，一条消息只会被其中一个消费者消费。它主要用于**任务削峰和横向扩容**，比如双十一批量处理订单，通过增加消费者节点来提升整体吞吐量。”
...
**工作原理：**
引入了 **Fanout（扇出）交换机**。生产者把消息发给交换机，交换机**忽略路由键（RoutingKey）**，直接把消息**广播**到所有绑定到它的队列中。每个消费者绑定自己独立的队列，因此所有消费者都能收到相同的消息。

- **结构**：P -> Fanout Exchange -> Q1, Q2 -> C1, C2（1对多广播）

> 🗣️ **面试回答（几句话讲清楚）：**
> “发布订阅模式使用了 **Fanout 交换机**，核心特点是**广播**。生产者将消息发给交换机，交换机会把消息复制到所有绑定的队列中，实现一条消息被多个不同系统同时消费。适用于**多系统联动**场景，比如用户注册后，同时触发发邮件、发短信、赠送积分三个独立服务。”
...
**工作原理：**
使用了 **Direct（直连）交换机**。生产者发送消息时必须携带一个路由键（RoutingKey），队列在绑定交换机时也需要指定一个绑定键（BindingKey）。交换机只会把消息路由到 **RoutingKey 和 BindingKey 完全匹配** 的队列中。

- **结构**：P -> Direct Exchange -> (按 RoutingKey 精确匹配) -> Q1, Q2 -> C1, C2

> 🗣️ **面试回答（几句话讲清楚）：**
> “路由模式使用了 **Direct 交换机**，核心特点是**精确匹配**。生产者发送消息时携带路由键，交换机只把消息路由到绑定了相同路由键的队列。适用于**按类型分发**的场景，比如日志收集系统中，`error` 级别的日志路由到报警队列，`info` 级别的日志路由到存储队列。”
...
**工作原理：**
使用了 **Topic（主题）交换机**。它是路由模式的升级版，支持**通配符模糊匹配**。

- 路由键必须是用小数点 `.` 分隔的单词列表（如 `order.create`）。
- 通配符规则：
  - `*`：匹配**恰好一个**单词（如 `order.*` 能匹配 `order.create`，不能匹配 `order.create.success`）。
  - `#`：匹配**零个或多个**单词（如 `order.#` 能匹配所有以 `order.` 开头的路由键）。
- **结构**：P -> Topic Exchange -> (按 RoutingKey 模糊匹配) -> Q1, Q2 -> C1, C2

> 🗣️ **面试回答（几句话讲清楚）：**
> “主题模式使用了 **Topic 交换机**，核心特点是**通配符模糊匹配**。它支持使用 `*` 匹配一个单词，`#` 匹配多个单词，实现更灵活的消息路由。适用于**复杂规则分发**的场景，比如电商系统中，`order.#` 可以订阅所有订单相关消息，而 `*.pay` 可以专门订阅所有支付相关的消息。”
...
> “这五种模式本质上可以归为两类：
>
> 1. **前两种（简单、工作队列）**：没有显式使用交换机（底层用的是默认的空交换机），消息直接发给队列，区别在于**消费者是一个还是多个**。
> 2. **后三种（发布订阅、路由、主题）**：显式使用了交换机，消息先发给交换机再路由到队列。它们的区别仅仅在于**交换机的类型不同**（Fanout 广播、Direct 精确匹配、Topic 模糊匹配），从而实现不同的消息分发规则。”
...
> 《后端面试高频系统设计&场景题》的这篇文章有详细介绍[订单超时自动取消如何实现？](https://www.yuque.com/snailclimb/tangw3/sgageiesw7cqi1o7) （星球专栏合集（地址和密码获取）：https://t.zsxq.com/avfM0 ）。一定要利用好星球的资料。
>



1. **RocketMQ 延迟消息 / RabbitMQ 延迟队列（推荐）**：下单成功后，立刻向 MQ 投递一条延迟消息（如 15 分钟）。15 分钟后，消费者准时收到消息并核对订单支付状态（状态机）。若未支付，则执行关单并回滚库存。库存回滚建议走补偿消息（再发一条 MQ），避免直接在消费者里操作 DB 导致事务爆炸。
2. **Redisson 延迟队列（备选）**：如果公司技术栈没有引入重量级的 MQ，可以使用 Redisson 延迟队列来实现，轻量且高效。Redis 虽然有 AOF/RDB，但在极端宕机情况下仍有丢数据风险。建议配合一个低频的（如每小时一次）Spring Task 扫表作为兜底，专门捞取漏处理的极少数超时订单。不过，极端高并发下，Redis 内存 + 调度线程容易成为瓶颈。另外，可靠性相对 MQ 稍弱，订单场景需要权衡。

**RocketMQ 延迟消息介绍**：

- RocketMQ 4.x 版本及其之前的版本支持基于预定义的延时等级的延时消息处理。消息发送者可以指定一个延时等级（如 1s、5s、10s 等），然后消息会在相应的延时级别到达后被发送到消费者队列。这些延时等级是固定的，不能灵活配置。
- RocketMQ 5.0 基于时间轮算法引入了定时消息，解决了延时级别只有 18 个、延时时间不准确等问题。RocketMQ 5.0 对延时消息的优化的详细介绍可以看看这篇文章：[弥补延时消息的不足，RocketMQ 基于时间轮算法实现了定时消息！](https://mp.weixin.qq.com/s/I91QRel-7CraP7zCRh0ISw)。

**RabbitMQ 延迟队列实现方式**：

1. DLX + TTL（RabbitMQ 3.6.x 之前）：将消息发送到一个设置了 TTL 的队列中，当消息过期后，会被转发到另一个设置了 DLX（Dead Letter Exchange，死信队列）的队列中，消费者监听这个 DLX 队列来获取延迟消息。
2. 延迟消息插件（RabbitMQ 3.6.x 开始，推荐）：RabbitMQ 官方提供了延迟队列插件 `rabbitmq_delayed_message_exchange`。声明 `x-delayed-message` 类型的 Exchange，消息发送时指定消息头 `x-delay`（毫秒）将消息进行延迟投递。插件支持的最大延迟时间：(2^32)-1 毫秒，约 49 天。相比 DLX + TTL，更简单方便，解决了消息阻塞问题。
...
小项目：不断地扫描数据库通过订单时间判断是否有超时的订单

* 单机环境：Spring Task Quartz ；

  ```java
  @Scheduled(fixedRate = 5 * 60 * 1000)// 5min执行一次
...
* 简单，支持集群操作，数据量小 且 不在乎时效性则可以考虑
* 不依赖第三方组件，实现方便

缺点

* 实时性较低（如果每个三分钟扫描一次，那么最坏的延迟时间就是三分钟）

* 假设订单有几千万条，每隔几分钟扫描一次，数据库损耗极大

* 订单表通常数据量极大。定时任务扫表往往意味着**全表扫描**（O(N) 复杂度）。如果你有用过 MySQL 的执行计划（`EXPLAIN`）进行分析，就会发现这种查询的 `type` 极容易退化为 `ALL`。这会引发**大量的磁盘 I/O**，导致**数据库 CPU 瞬间飙升至 100%，甚至引发锁表，直接拖垮主库，影响核心链路**。

* 定时任务的执行周期**存在时间差**（例如每分钟执行一次），会导致**库存释放不及时**，严重影响真实用户的购买转化率。

  

> #### 优化点
>
> 如果必须一定要用定时任务扫表，还可以做一些针对性优化，例如：你可以通过在 SQL 中增加时间范围限制，配合合适的索引，将全表扫描（O(N)）优化为局部索引扫描。比如，只查询过去 30 分钟内未支付的订单：`SELECT id FROM order WHERE status = 0 AND create_time >= DATE_SUB(NOW(), INTERVAL 30 MINUTE);` 并且在数据库中建立 `(status, create_time)` 的联合索引。这样，查询的 `type` 就会从 `ALL` 变成 `range`，极大减少扫描的数据量。
>
> 虽然这种做法解决了全表扫描的致命伤，但依然存在以下几个无法回避的痛点（你可以把这些点作为面试时和面试官拉扯的深度素材）：
>
> 1. **瞬时数据量依然巨大，数据库压力并未根除**：秒杀的特点是"短时间内涌入海量订单"。即使你只扫描最近 5 分钟的订单，这 5 分钟内也可能产生了十万级甚至百万级的订单记录。定时任务将这十几万条数据从 MySQL 查出来，加载到应用内存中，再逐个判断、修改状态，这个过程对数据库的 CPU、磁盘 I/O 以及应用服务器的内存（极易引发 Full GC）依然是巨大的负担。<u>而秒杀期间，数据库的性能应该全部让步于核心的"下单扣库存"链路</u>。
> 2. <u>**时效性差，导致"黄金库存"被白白占用**</u>：定时任务本质上是"轮询（Pull）"机制，必定存在时间差。假设你设置每 1 分钟执行一次扫描任务，订单 15 分钟超时。如果一个订单在第 15 分钟零 1 秒超时，它可能要等到第 16 分钟定时任务触发时才会被关闭并释放库存。在秒杀场景下，库存就是生命线。这 1 分钟的延迟，意味着真正想买的用户看到的是"已售罄"，等 1 分钟后库存释放了，用户早就流失了，严重影响转化率。
> 3. **分布式环境下的并发冲突与重复执行**：现在的服务都是集群部署（多个节点）。如果只是简单用 Spring Task 的 `@Scheduled` 注解，集群中的 3 台机器会在同一时间触发扫表任务。这就需要引入分布式锁或者使用 XXL-JOB 这样的分布式任务调度框架来进行分片广播。无形中把一个简单的关单逻辑搞得非常复杂，且增加了极大的锁争用开销。



##### 分布式任务调度平台

Elastic-Job / XXL-Job / SchedulerX

优点：

* 支持分片处理，可以自定义规则  配置每台机器处理不同规则的订单，将单台处理分散到多台处理
* 高效处理大规模数据，提高系统的可用性和容错能力
...
使用 JDK 自带的 DelayQueue实现，

> DelayQueue 是一个**无界阻塞队列**，该队列只有在延迟期满的时候才能从中获取元素，放入 DelayQueue中的对象，是必须实现 Delayed 接口的

DelayQueue 实现的工作流程：

![image-20260311114312481](assets/image-20260311114312481.png)

> poll()：获取并移除队列的超时元素，没有则返回空
>
> take()：获取并移除队列的超时元素，<u>如果没有则等待</u>当前线程，直到有元素满足超时条件，返回结果

优点：

* 效率高，任务触发时间延迟低 (因为 take 操作会一直等待任务)
* 不依赖第三方组件，实现方便

缺点：

* 服务器重启后，数据全部消失
* 集群扩展性相当麻烦（多个jvm内存无法共享）
* 内存条件限制（如果下单未付款的订单数太多，那么很容易出现 OOM ）
* 代码复杂度较高
...
![image-20260311115037304](assets/image-20260311115037304.png)

![image-20260311115103812](assets/image-20260311115103812.png)

轮转到哪个槽位，就执行哪个槽位的功能

优点：

* 效率高，任务出发时间延迟时间比 DelayQueue 低，代码复杂度比 DelayQueue 低
...
https://www.bilibili.com/video/BV1f1421m7qc/?spm_id_from=333.1245.top_right_bar_window_history.content.click&vd_source=67ef3bb4c8d68a96408acdaa865b1313【还没整理完】

https://www.bilibili.com/video/BV1WSyrBKE9h/?spm_id_from=333.788.top_right_bar_window_history.content.click&vd_source=67ef3bb4c8d68a96408acdaa865b1313
