...
| 8. 枚举             | ENUM(‘A’,’B’)                    | CREATE TYPE mood AS ENUM(‘A’,’B’) 或 CHECK + VARCHAR     | 直接不支持                                            |
| 9. 布尔             | TINYINT(1) 0/1                   | BOOL 或 BOOLEAN                                          | 都能存 0/1，但 PG 直接返回 true/false，ORM 可能映射错 |
...
#### 结论
“增删改查 + 普通 JOIN” 基本无缝；  
**一旦用到自增、分页、函数、Upsert、枚举、反引号**，就必须改。  
...
| **日期时间** | DATE / TIME / DATETIME\[(fsp)] | DATE / TIME / TIMESTAMP\[(p)]              | ③ `DATETIME` → `TIMESTAMP(0)` 去掉小数秒；`TIMESTAMP` 带时区用 `TIMESTAMPTZ` |
| **年份**     | YEAR                           | 无直接对应                                 | 用 `DATE` 或 `SMALLINT` 存 4 位年份                          |
| **枚举**     | ENUM('A','B','C')              | CREATE TYPE mood AS ENUM('A','B','C')      | ④ 先建自定义类型，再字段用 `mood`；Navicat 传输向导可自动完成 |
| **布尔**     | TINYINT(1) 0/1                 | BOOLEAN                                    | **语义层**：想继续存 **-128~127** 的小整数 → 改成 `SMALLINT(即int2)`； **布尔层**：本来就当 0/1 用 → 直接改 `BOOLEAN(BOOL)`，最干净； |
...
- **JSONB、UUID、数组、范围、IP 类型** 是 PG 彩蛋，用了就回不去。
...
> 只有 3 个空格“必须”填/选，其余全部留空（保持 Navicat 给的默认值）即可
>
> | 字段                | 要不要填 | 怎么填/选                                           |
> | ------------------- | -------- | --------------------------------------------------- |
...
> | 是模板              | ❌留空    | 不要勾，普通业务库用不到                            |
>
> 填完“数据库名称 + 所有者 + 编码”点“确定”即可，其他空格一律无视。
...
1. 右键任意表 → 转储 SQL 文件 → 结构+数据 → 保存为 `employee.sql`，一键拿到纯 SQL，方便 Git 做版本。  
...
- **外键图标看不见**：Navicat 默认把“引用”折叠在“约束”页签里，需手动展开。  
- **serial 字段插数据时报错**：图形界面插行时把 `id` 留空（<null>），让序列自增即可，不要填 0。  
- **schema 搜索路径**：若新建了 schema，查询里最好带前缀：`SELECT * FROM sales.employee;` 或在连接属性 → 高级 → 搜索路径里加 `sales`。

照以上 6 步，你就可以用 Navicat 完成“连接 → 建库 → 建表 → 插数 → 查数 → 导出”全套日常开发流程。
...
mybatis:
  ...
  type-aliases-package: top.hazenix.auris.entity
  configuration:
    #开启驼峰命名
